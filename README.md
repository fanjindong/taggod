...
| 能力 | 说明 |
| --- | --- |
| 搜索切换 | 输入标题、网址、主域名、分组名、多个关键词或英文缩写，在所有已打开窗口中查找标签页。 |
...
打开弹窗后，搜索框会自动聚焦。直接输入关键词，就能按标题、网址、主域名或自定义分组名查找标签页。搜索支持空格分隔的多个关键词，也支持英文标题、分组名和网址片段的首字母缩写。例如输入 `github issue` 可以缩小到同时命中 GitHub 和 issue 的页面，输入 `gpr` 可以命中 `GitHub Pull Requests` 这类英文标题。
...
没有输入关键词时，弹窗只展示最近使用的已打开页面，避免历史结果干扰快速切换。输入关键词后，才会搜索最近关闭的标签页。最近关闭记录发生变化后会自动刷新；如果结果来自一个关闭的窗口，界面会明确标出窗口内的标签数量，选择其中任一结果都会按 Chrome 原生行为恢复整个窗口。
